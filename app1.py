...
#Mongodb setup
client = pymongo.MongoClient("mongodb+srv://jurrunirosha:Nirosha%4021@cluster0.swmsfey.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
...
def docverify():
        ...
        session['user']=doc['idv']
        #sess.username=doc['idv']
...
def available():
    ...
    for x in mydat:
        mydata=db[x['owner']]#record of patient
        mycl=mydata.find()
        for y in mycl:
            if x['record']==y['_id']:
                if x['status']==1:
